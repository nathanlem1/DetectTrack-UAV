...
def main(args):
    ...
    for seq_name in seqs:
        ...
        gt_file = os.path.join(DATA_ROOT, 'UAV-benchmark-MOTD_v1.0', 'GT', str(seq_name) + '_' + GT_FILE)

        # gt_ignore_file = os.path.join(DATA_ROOT, 'UAV-benchmark-M', seq_name, 'gt', GT_IGNORE_FILE)
        gt_ignore_file = os.path.join(DATA_ROOT, 'UAV-benchmark-MOTD_v1.0', 'GT', str(seq_name) + '_' + GT_IGNORE_FILE)

        # gt_new_file = os.path.join(DATA_ROOT, 'UAV-benchmark-M', seq_name, 'gt', GT_NEW_FILE)
        gt_new_file = os.path.join(DATA_ROOT, 'UAV-benchmark-MOTD_v1.0', 'GT', str(seq_name) + '_' + GT_NEW_FILE)

        # First copy the gt file
        # subprocess.run(['cp', gt_file, gt_new_file])
        shutil.copyfile(gt_file, gt_new_file)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_root', type=str,
                        default='C:/Users/nbaisa00/PycharmProjects/PythonProjects/datasets/UAVDT')
    parser.add_argument('--store_file_name', type=str, default='gt_merge.txt',
                        help='Merging _gt.txt and _gt_ignore.txt')
